...
class SMS_module
{
    public static function send($receiver, $otp)
    {
        ...
        $config = self::get_settings('excite_sms');
        if (isset($config) && $config['status'] == 1) {
            $response = self::excite_sms($receiver, $otp);
            return $response;
        }
    ...
    public static function msg91_sms($receiver, $otp)
    {
        $config = self::get_settings('msg91_sms');
        $response = 'error';
        if (isset($config) && $config['status'] == 1) {
...
//excite
    public static function excite_sms($receiver, $otp)
    {
        $config = self::get_settings('excite_sms');
        $response = 'error';
        if (isset($config) && $config['status'] == 1) {
            ...
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://gateway.excitesms.tech/api/v3/sms/send?recipient' . $receiver . $config['sender_id'] . '&message' . $message. '',
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
            ...
            DB::table('business_settings')->updateOrInsert(['key' => 'excite_sms'], [
                'key' => 'excite_sms',
                'value' => json_encode([
                    'status' => 0,
                    'sender_id' => '',
